...
- 承辦人帳號只能由管理員建立、啟用或停用。
- 系統允許同時存在多位啟用中的承辦人，可用於交接、備援或多人共同處理待審申請。
- 管理員不應交接同一組帳號密碼，必須使用管理員移交流程。
- 同一時間只能存在一位啟用中的管理員。管理員移交時，先讓新管理員透過 Email 完成密碼設定，但帳號維持未啟用；再於同一個 PostgreSQL Transaction 中停用舊管理員並啟用新管理員。
...
- 新管理員透過 Email 啟用連結設定自己的密碼。若系統已存在啟用中的管理員，完成密碼設定後只寫入 `password_hash` 與 `activated_at`，帳號仍維持 `is_active = FALSE`。
- 目前管理員確認新管理員已完成密碼設定後，執行管理員移交。
- 移交必須在同一個 PostgreSQL Transaction 中完成：鎖定舊管理員與新管理員、撤銷舊管理員有效 session、停用舊管理員、啟用新管理員，並寫入 `admin.transferred` 通用稽核紀錄。
- 移交目標必須是 `role = 'admin'`、`activated_at IS NOT NULL` 且 `is_active = FALSE` 的帳號。
- `one_active_admin` Partial Unique Index 保證同一時間最多只有一位啟用中的管理員。

管理員忘記密碼：

- 若管理員仍可收取帳號 Email，應使用一般密碼重設流程。
- 只有已完成首次 activation 且已有密碼的管理員帳號才使用密碼重設；尚未完成 activation 的管理員帳號應由現任管理員或維運指令重寄啟用信。
- 密碼重設成功後必須撤銷該管理員既有 session，並清除密碼重設 token hash。
- 已完成 activation 但尚未啟用的新管理員帳號可以重設密碼，但重設成功後仍維持 `is_active = FALSE`，必須透過管理員移交流程啟用，不得用密碼重設繞過移交。

唯一管理員無法登入：

- 不提供公開管理員復原 API，避免被外部濫用。
- 應使用只能在伺服器執行的維運指令，例如 `npm run admin:recover -- admin@example.com`。
- 維運指令應驗證目標帳號是目前唯一管理員或符合受控復原條件，撤銷相關 session，產生限時密碼重設連結並寄送到管理員 Email。
- 若原管理員 Email 已不可用，必須先經校內或維運人工驗證，再由受控維運流程更新管理員 Email 或建立替代管理員；該操作不得透過公開 API 執行。
- 管理員復原必須寫入 `maintenance.admin_recovered` 或 `admin.recovered` 通用稽核紀錄。
...
第一版不使用 Redis 作為權限來源。Redis 可用於 session、rate limit、登入失敗計數或短期 cache，但不應作為角色權限設定的唯一真相來源，避免 Redis 清空、過期或誤寫造成權限錯亂。

若未來需要動態權限，應以 PostgreSQL 的 `roles`、`permissions`、`user_roles`、`role_permissions` 等資料表作為 source of truth；Redis 只能作為查詢快取，例如快取某角色或某使用者的權限集合，並需設計失效與重新載入策略。
...
權限總覽（共 56 個權限代碼，包含第二版預留代碼）：

- 指導老師 8 個
- 承辦人第一版 11 個，另有 2 個第二版點數異動預留代碼
- 管理員第一版 31 個，另有 7 個第二版點數異動、稽核查詢與 Email task 管理預留代碼

第二版預留代碼可以先保留在型別與角色映射中，方便後續開發，但第一版不得掛載對應 route。單有權限代碼不代表功能已對外開放。
...
系統允許多位承辦人帳號，可用於交接、備援或多人共同處理待審申請。所有承辦人採自由搶案模式。
...
| `point_change_requests.list` | 第二版：查看所有核准後點數異動申請歷史 |
| `point_change_requests.create` | 第二版：建立核准後點數異動或沖銷申請 |
...
主任異動與停用規則：只有管理員具有 `advisors.assign_director` 權限。現任主任不可直接停用，必須先將另一位啟用中的指導老師指定為主任；主任切換需在同一個 PostgreSQL Transaction 中完成。
...
| `users.transfer_admin` | 將唯一啟用中的管理員移交給已完成密碼設定的新管理員 |
...
| `participant_rules.list` | 查看目前及歷史申請人數規則 |
| `participant_rules.create` | 建立未來生效的人數規則版本 |
| `participant_rules.deactivate` | 設定舊人數規則失效日期 |
| `application_instructions.list` | 查看公開及未公開的申請說明 |
| `application_instructions.create` | 建立申請說明 |
| `application_instructions.update` | 更新允許修改的說明欄位 |
| `application_instructions.visibility` | 顯示或隱藏申請說明 |
| `point_change_requests.list` | 第二版：查看所有核准後點數異動申請 |
| `point_change_requests.read` | 第二版：查看異動申請詳細資料 |
| `point_change_requests.approve` | 第二版：核准異動申請並建立點數流水帳 |
| `point_change_requests.reject` | 第二版：拒絕異動申請 |
...
| `audit_logs.read` | 第二版：查看通用系統稽核紀錄 |
| `email_tasks.read` | 第二版：查看 Email 任務與永久失敗紀錄 |
| `email_tasks.retry` | 第二版：手動重寄 failed email task |
| `student_point_transactions.read` | 查看學生點數流水帳 |

允許：

- 建立、啟用、停用與管理使用者帳號，並執行管理員移交。
- 管理指導老師資料及主任身分。
- 版本化管理點數規則與申請人數規則。
- 建立、排序、顯示或隱藏申請說明；已生效內容需以新資料保留歷史。
- 唯讀查看所有申請、附件、指導老師簽名、審核紀錄與學生點數流水帳。
- 第二版啟用後，查詢稽核紀錄、管理 Email tasks，以及處理核准後點數異動申請。
...
| 核准前調整認定與點數 | 不可 | 可 | 不可 |
| 核准或拒絕一般申請 | 不可 | 可 | 不可 |
| 建立核准後點數異動申請 | 不可 | 第二版 | 不可 |
| 核准或拒絕點數異動申請 | 不可 | 不可 | 第二版 |
| 直接修改點數流水帳 | 不可 | 不可 | 不可 |
| 管理點數規則 | 不可 | 不可 | 可 |
| 管理申請人數規則 | 不可 | 不可 | 可 |
| 管理申請說明 | 不可 | 不可 | 可 |

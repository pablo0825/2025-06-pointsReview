...
| `audit_logs.read` | 查看通用系統稽核紀錄 |
| `email_tasks.read` | 查看 Email 任務與永久失敗紀錄 |
| `email_tasks.retry` | 手動重寄 failed email task |
...
- 查詢通用系統稽核紀錄。
- 查看 failed email tasks，並手動建立重寄任務。

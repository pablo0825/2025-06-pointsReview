...
| Service integration test | 業務規則、Transaction、狀態轉換、email task/audit log 建立 | 是 | 送件、簽核、補件、核准、規則管理 |
...
- 查詢 Schema：
  - Advisor pending 只接受分頁，history status 只接受允許的非 pending 狀態。
  - Reviewer review / history 的狀態與日期區間限制。
  - Admin user id、user list 與 advisor list query 驗證。
- storage key helper：
  - 安全字元。
  - 不接受路徑穿越。
...
- `application_instructions`：
  - 同一 `(applicationType, sectionKey)` 可保存不同生效日版本，但有效期間不可重疊。
  - 歷史版本、顯示狀態與排序查詢正確。
...
- `student_point_transactions`：
  ...
  - snapshot 欄位必填。
- `student_point_change_requests`（第二版預留 schema）：
...
- 簽名檔案寫入失敗時整個操作失敗。
- Pending 只包含目前登入老師且狀態為 `pending_advisor` 的申請。
- History 只包含登入老師已處理、目前不在 `pending_advisor` 的申請。
- 補件重新提交後申請回到 pending，舊版本與舊簽核仍可由詳情歷史查詢。
- Pending / history detail 都不能讀取其他老師的申請。
...
- Review queue 與 detail 只接受 `under_review`、`needs_revision`。
- Reviewer history 只包含 `approved`、`rejected`，並可讀取完整版本摘要與審核操作紀錄。

### 規則與申請說明管理

測試案例：

- 點數規則與人數規則建立接續版本時，在同一 Transaction 關閉舊版本並建立新版本。
- 規則期間重疊時回傳穩定錯誤且不留下半完成資料。
- 申請說明已生效後不可原地修改內容，尚未生效版本可修改。
- 公開說明預設只回傳目前有效內容；`includeHistorical = true` 包含已過期公開內容，但不回傳未來或隱藏內容。
- 規則與說明異動建立對應 `audit_logs`。
...
- Public：
  ...
  - 公開點數查詢只回傳 masked 欄位。
- Advisor：
  - Pending / history 列表的分頁、狀態分類與排序正確。
  - Pending / history detail 只能讀取自己負責的申請。
  - 老師不能查看其他老師申請。
  - 老師簽名成功。
  - 老師拒絕必須有 reason。
- Reviewer：
...
- Admin:
  ...
  - User detail 不回傳密碼、token hash 或 session 資料。
  - Advisor list 的啟用狀態、主任與關鍵字篩選正確。
  - 點數規則、人數規則與申請說明管理 API 需要 admin 權限。
  - 管理員不能核准一般申請。
- Private file:
  - 未登入不可讀附件。
  - 無資料範圍不可讀附件。
...
- 第一版沒有 Audit log 查詢 route；必要事件仍會正確寫入。
...
7. API tests for auth, public application, advisor approve, reviewer approve, admin user/advisor/rule/instruction management。
...
- 核准後點數異動與沖銷的 Service / API tests。
- Audit log 管理查詢 API tests。
- Email task 管理查詢與手動 retry API tests。

Browser E2E、視覺 snapshot、壓力測試與自動化復原演練可在前端及部署環境穩定後補上；點數異動、Audit log 查詢與 Email task 管理測試隨第二版功能一起實作。

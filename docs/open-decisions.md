...
- 除 `users`、`advisors`、`point_applications`、`application_participants`、`application_versions`、`advisor_signatures`、`application_attachments`、四張點數規則資料表與四張類型專屬資料表外，其餘資料表套用共用型別規範後，各欄位的最終型別與 `NULL` 限制。

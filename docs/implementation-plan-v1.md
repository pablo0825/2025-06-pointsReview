...
- [x] 建立 `EmailTaskRepository`：
  - [x] 在業務 Transaction 中建立 pending task。
  - [x] 使用 `FOR UPDATE SKIP LOCKED` claim `scheduled_at <= NOW()` 的 pending tasks。
  - [x] 標記 `sent` 並寫入 `sent_at`。
  - [x] 寄送失敗時增加 `attempt_count` 並寫入安全處理後的 `last_error`。
  - [x] 未達重試上限時重新排程為 `pending`，達上限時標記為 `failed`。
- [x] 建立 `EmailTaskService`、template renderer interface 與 `EmailProvider` interface；使用 fake adapter 驗證核心流程，Service 與 worker 不直接綁定特定寄信套件。
- [x] 建立可單次執行的 worker function；Phase 4.2 不直接整合常駐排程器，Phase 10 的 worker lifecycle 只負責定期呼叫此 function。
- [x] 確保 Email payload、application log 與 `last_error` 不保存或輸出密碼、token hash、SMTP credential；原始一次性 token 只可存在需要寄出的連結 payload，寄送與錯誤 log 不得輸出。
- [x] 補 Email task 與 worker 測試：
  - [x] 業務 Transaction rollback 時不會留下 pending task。
  - [x] 只 claim 已到期的 pending task，平行 worker 不會取得同一筆 task。
  - [x] 使用 fake renderer 與 fake provider 驗證成功投遞。
  - [x] 寄送成功、可重試失敗與永久失敗狀態轉換。
  - [x] Email payload、log 與 `last_error` 不洩漏敏感資料。
...
- [x] 建立共用 `passwordSchema`：
  - [x] 驗證字串型別、至少 `12` 字元、最多 `128` 字元。
- [x] 建立 `PasswordPolicy`：
  - [x] 禁止常見弱密碼。
  - [x] 不允許與 Email local part 完全相同。
- [x] 建立一次性帳號 Token 基礎：
  - [x] 使用密碼學安全亂數產生 activation / password reset 原始 token。
  - [x] 資料庫只保存 SHA-256 token hash 與到期時間。
  - [x] 驗證 token 格式、hash、期限與使用後失效。
- [x] 建立 account activation 與 password reset Email template mapping。
- [x] 實作正式帳號建立與管理流程：
  - [x] 建立初始管理員維運指令，產生 activation token、email task 與 audit log。
  - [x] `UserAdminService.createUser` 建立帳號、activation token 與 email task。
  - [x] activate user；deactivate user 沿用 Phase 4.1 已完成的流程。
  - [x] resend activation，產生新 token 並使舊 token 失效。
  - [x] send password reset，產生 password reset token 與 email task。
  - [x] transfer admin。
  - [x] `AdvisorAdminService.createAdvisor` 建立指導老師與登入帳號關聯。
  - [x] `POST /admin/users` 只建立 `admin`、`reviewer`；`advisor` 必須由 `POST /admin/advisors` 同時建立帳號與老師資料。
- [x] 實作帳號生命週期管理 API：
  - [x] `POST /admin/users`
  - [x] `POST /admin/users/:userId/transfer-admin`
  - [x] `POST /admin/users/:userId/resend-activation`
  - [x] `POST /admin/users/:userId/send-password-reset`
  - [x] `POST /admin/advisors`
- [x] 實作 `POST /auth/activation/:token`：
  - [x] 驗證 activation token hash 與到期時間。
  - [x] 套用共用密碼 schema 與 policy。
  - [x] 設定 Argon2id password hash、清除 token 並寫入 `activated_at`。
  - [x] 依初始管理員／管理員移交流程判斷是否啟用帳號。
- [x] 實作 `POST /auth/password-reset/request`：
  - [x] 不揭露 Email 是否存在。
  - [x] 只有已完成首次 activation 且已有密碼的帳號才產生 password reset token hash、到期時間與 email task。
  - [x] 尚未完成 activation、尚未設定密碼、不存在或狀態不符合的帳號仍回傳相同成功 response。
- [x] 實作 `POST /auth/password-reset/:token`：
  - [x] 驗證 password reset token hash 與到期時間。
  - [x] 套用共用密碼 schema 與 policy。
  - [x] 更新 Argon2id password hash、清除 token 並撤銷既有 session。
  - [x] 不修改 `is_active`；已停用但完成 activation 的帳號重設後仍維持停用。
  - [x] 第一版不檢查新密碼是否與目前密碼相同；密碼重用限制列入第二版候選功能。
  - [x] 建立 `user.password_reset_completed` audit log。
- [x] 補正式帳號生命週期測試：
  - [x] 帳號建立、重寄啟用與密碼重設會建立正確且不重複的 email tasks。
  - [x] Activation / reset token 過期、使用後失效與密碼規則。
  - [x] Password reset 不洩漏 Email，成功後撤銷 session 並建立 audit log。
  - [x] 未完成 activation 的帳號不建立 password reset task；已停用但完成 activation 的帳號可重設密碼，完成後仍維持停用。
  - [x] 未完成首次密碼設定的帳號不能透過 Phase 4.1 activate API 繞過 activation。
  - [x] 管理員移交符合唯一啟用管理員限制，並撤銷舊管理員 session。
...
- Phase 4.3 建立正確 Email template、URL、payload 與 task；正式 Email provider 仍依 Phase 10 的服務選型與 worker 維運整合處理。
- Activation 與 password reset 的 Redis rate limit 依本計畫集中於 Phase 9 實作。

驗收紀錄：

- `npm run build` 通過。
- `npm test`：19 個 test files、95 個 tests 全數通過。
- `npm run migrate:status`：migration files 21、applied migrations 21、pending migrations 0。
...
- multipart 檔案欄位使用 `attachments[{clientFileKey}]`，明確對應 payload 的附件 metadata。
- 新上傳先寫入 OS 臨時目錄，驗證成功後才移入私有儲存；成功與失敗都必須清除臨時檔案。
...
- [x] 完成 Phase 4.2 通用 Email task、單次 worker 與有限重試核心。
- [ ] 需要正式帳號 onboarding 時進入 Phase 4.3。

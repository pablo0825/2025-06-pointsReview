...
- [ ] 實作正式帳號建立與管理流程：
  ...
  - [ ] `POST /admin/users` 只建立 `admin`、`reviewer`；`advisor` 必須由 `POST /admin/advisors` 同時建立帳號與老師資料。
- [ ] 實作帳號生命週期管理 API：
  - [ ] `POST /admin/users`
...
- [ ] 補正式帳號生命週期測試：
  ...
  - [ ] 未完成 activation 的帳號不建立 password reset task；已停用但完成 activation 的帳號可重設密碼，完成後仍維持停用。

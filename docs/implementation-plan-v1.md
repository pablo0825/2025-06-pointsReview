...
已確認行為：重複建立相同 `event_key` 且收件人、模板、payload、申請關聯、排程與重試上限一致時採冪等成功並回傳既有 task；內容不一致時視為程式錯誤，不覆蓋既有 task。成功寄送保留 `attempt_count`、清除 `last_error`。Renderer 或 payload 錯誤與 provider 明確永久錯誤不可重試；timeout、rate limit、network、provider 5xx 與無法辨識的 provider 錯誤預設可重試。單次 worker 預設最多處理 `10` 筆，但每次只 claim 一筆，完成該筆狀態更新後才 claim 下一筆。

...
指導老師與承辦人的列表、詳情 endpoint 使用共用 primitive 組合成各自的 query / params schema，不共用一個接受所有角色欄位的大型 schema。

共用 `applicationPublicIdParamsSchema` 驗證 `publicId` 為 UUID。Zod 只驗證格式；申請是否存在、目前角色是否可讀取，以及狀態是否符合 endpoint 範圍由 Service 驗證。

`advisorApplicationHistoryQuerySchema` 驗證：

- `applicationType` 可選，必須是四種申請類型之一。
- `status` 可選，只接受 `under_review`、`needs_revision`、`approved`、`rejected`。
- `submittedFrom`、`submittedTo` 為 ISO 8601 date-time，且起始時間不得晚於結束時間。
- 使用共用 `page`、`pageSize`，最大 `pageSize = 100`。

Advisor pending query 只使用共用分頁，不接受由前端指定 `advisorId` 或任意狀態；登入老師與 `pending_advisor` 範圍由 Service 固定加入查詢。

Reviewer query 分開定義：

- `reviewerReviewQuerySchema` 可接受 `applicationType`、`keyword`、送件日期區間與分頁，狀態由 Service 固定為 `under_review`、`needs_revision`。
- `reviewerHistoryQuerySchema` 額外接受可選 `status`，只允許 `approved`、`rejected`。
- Reviewer detail params 使用 `applicationPublicIdParamsSchema`；Zod 不負責判斷申請是進行中或已結案，對應 Service 必須依 endpoint 重新檢查。

管理端查詢使用獨立 schema：

- `adminUserIdParamsSchema` 驗證 `userId` 為正整數。
- `adminUserListQuerySchema` 驗證角色、啟用狀態、關鍵字與分頁。
- `adminAdvisorListQuerySchema` 驗證 `keyword`、`isActive`、`isDirector` 與分頁。

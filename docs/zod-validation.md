...
後台審核、規則管理與點數異動也應使用 Zod 驗證 request 格式：
...
| 點數異動申請 | change type、target transaction id、requested points | 目標交易是否可異動、pending 衝突、異動後點數不可小於 0 |
| 規則管理 | enum、點數範圍、有效日期區間 | 有效期間不可重疊、是否影響未來申請 |

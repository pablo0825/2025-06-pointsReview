...
export const getPointsData = async (req: Request, res: Response) => {
    ...
    const query = {
        year,
        isLocked: false,
        $or: [{studentId: {$regex: keyword, $options: "i"}}, {name: {$regex: keyword, $options: "i"}}]
    }

    const user = await PointsTableDB.find(query).select("studentId name isComplete group status").sort({studentIs: 1});
